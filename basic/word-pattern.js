...
var wordPattern = function (pattern, s) {
  ...
  for (let [i, code] of sArr.entries()) {
    const ch = pattern[i];
    if (
      (stackone.has(code) && stackone.get(code) !== ch) ||
      (stackTwo.has(ch) && stackTwo.get(ch) !== code)
    ) {
      return false;
    }
...
var wordPattern = function (pattern, s) {
  ...
  for (let [i, item] of sArr.entries()) {
    if (
      (stringMap.has(item) && stringMap.get(item) !== pattern[i]) ||
      (patternMap.has(pattern[i]) && patternMap.get(pattern[i]) !== item)
    ) {
      return false;
    }
